...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleDataIntegrityViolation(DataIntegrityViolationException ex) {
        ...
        if (rootMsg.contains("duplicate key value violates unique constraint")) {

            errorTitle = "Duplicate Value";
            Optional<String> columnName = extractColumnName(rootMsg);
            Optional<String> duplicateValue = extractBetweenParentheses(rootMsg);
            errorDetail = duplicateValue.isPresent() && columnName.isPresent()
                    ? String.format("Field '%s' with value '%s' already exists. Must be unique.", columnName.get(), duplicateValue.get())
        ...
        // 2. Handle foreign key violations
            ...
            errorDetail = refValue.map(s -> String.format("Invalid reference: %s doesn't exist", s))
                    .orElse("Referenced entity doesn't exist. Please check your references.");
...
//    }
    ...
    private Optional<String> extractColumnName(String message) {
    ...
    private Optional<String> extractBetweenParentheses(String message) {
